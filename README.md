...
| Commander_Casting | `/ccast`, `/cring` | Two ways to stop staring at a cast bar. The edges of the screen glow brighter as your cast completes, so you can feel it finish; and the cast can be drawn as an arc around the trim of the portraits — yours on the player frame, your target's on theirs, which is the one you watch when you are holding an interrupt. Colored by spell school (frost = blue, fire = red...), with a label for the seconds left, a latency mark, a global cooldown ring, and a switch to hide Blizzard's own cast bars entirely. |
